...
def load_mhpv1_dataset(image_path: Path, human_id_path: Path):
    datadict = []
    files = sorted(image_path.glob('*.jpg'))
    logger.info(f"loading MHP V1 dataset, glob {len(files)} images...")
...
if __name__.endswith('.mhp_v1'):
    _root = Path(os.getenv("DETECTRON2_DATASETS", "datasets"))
    register_mhpv1_instance('mhp_v1', _root / 'LV-MHP-v1')
